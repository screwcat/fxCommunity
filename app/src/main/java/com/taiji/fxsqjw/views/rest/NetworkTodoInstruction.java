...
public class NetworkTodoInstruction extends AsyncTask<String, Void, byte[]> {
    private Context context;
    private ProgressDialog pDialog;
    private List totalList = null;
    private InstructionListViewAdapter adapter = null;

    public NetworkTodoInstruction(Context context, List totalList, InstructionListViewAdapter adapter){
    ...
    protected void onPreExecute() {
        ...
        item.put("rowid","268");
        item.put("publisher","市局支队");
        item.put("instTitle","卡时间倒计时口");
        item.put("recTime","2017/10/10  08:50:25");
        totalList.add(item);
    ...
    protected void onPostExecute(byte[] result) {
        ...
        if (result != null) {
            ...
            try {
                ...
                HashMap<String,String> item = new HashMap<String,String>();
                item.put("rowid","268");
                item.put("publisher","123");
                item.put("instTitle","456");
                item.put("recTime","789");
                totalList.add(item);
                //totalList.addAll(list);
                adapter.notifyDataSetChanged();
            } catch (Exception e) {
